...
### 스키마
- **스키마**는 데이터 구조와 제약 조건을 정의하는 것이다.      
   예를 들어, 학생 테이블에 '학생ID', '이름', '학년'과 같은 필드가 포함된 구조를 정의하는 것이 스키마이다.
- 실제로 이 스키마에 맞게 저장된 객체를 **인스턴스**라고 한다.     
  예를 들어, 특정 학생의 정보는 '학생ID = 123', '이름 = 홍길동', '학년 = 2'와 같이 저장된 데이터 인스턴스이다.
- 스키마는 한 번 정의되면 자주 변경되지 않지만, 인스턴스는 데이터가 입력되거나 수정될 때마다 계속 변화한다.

### 3단계 데이터베이스 구조

미국의 표준화 기관인 ANSI는 데이터베이스의 내부 구조를 감추고 일반 사용자가 데이터베이스를 쉽게 이해하고 사용할 수 있도록 3단계 데이터베이스 구조를 제안했다. 
내부단계에서 외부단계로 갈 수록 추상화 정도가 높아진다.(내용을 숨기고 단순화된 정보 제공)
이 구조는 다음과 같다:
...
2. **DML (데이터 조작어)**
   ...
   -      절차적 데이터 조작어는 데이터를 얻기 위한 구체적인 과정을 설명하는 방식이다.    
          예를 들어, `INSERT`, `UPDATE`, `DELETE`는 데이터에 대한 명확한 변경 과정을 명시한다.
          비절차적 데이터 조작어 (선언적 언어)는 "무엇을 원하는지"만을 정의하고, 그 과정은 시스템에 맡기는 방식이다.
          예를 들어, `SELECT`는 원하는 데이터를 "선언"할 수 있다.
...
#### 1. **DDL 컴파일러**
   - **기능**: 테이블 생성, 수정, 삭제를 반영하고 데이터 사전을 업데이트한다.
   **예시**: `CREATE TABLE` 명령어 실행 시 테이블 구조를 정의한다.

#### 2. **DML 프리 컴파일러**
   - **기능**: DML 명령어를 응용 프로그램 코드와 결합하여 실행 준비를 한다.
   **예시**: C 프로그램에서 `EXEC SQL`로 SQL 쿼리를 작성한다.

#### 3. **DML 컴파일러**
   - **기능**: DML 명령어를 내부 형식으로 변환하여 실행 계획을 수립한다.
   **예시**: `SELECT` 쿼리를 내부 명령어로 변환하여 실행한다.

#### 4. **런타임 데이터베이스 처리기**
   - **기능**: 데이터베이스 쿼리를 실행하고 결과를 반환한다.
   **예시**: 사용자가 요청한 `SELECT` 쿼리 결과를 조회하여 반환한다.

#### 5. **트랜잭션 관리자**
   - **기능**: 트랜잭션의 원자성, 일관성, 격리성, 지속성을 보장한다. (ACID)
   - **예시**: 여러 쿼리가 포함된 트랜잭션이 완료되었는지 확인하고 롤백한다.
---
